...
const AuthenticatedVideoPlayer = ({ videoUrl, onError, className = "" }) => {
    ...
    const loadVideo = async () => {
      try {
        ...
        if (response.ok) {
          ...
          throw new Error(
            `Failed to load video: ${response.status} ${response.statusText}`
          );
        }
      } catch (err) {
  ...
  if (isLoading) {
    return (
      <div
        className={`w-full h-64 bg-gray-100 dark:bg-gray-700 rounded-lg flex items-center justify-center ${className}`}
      >
  ...
  if (error) {
    return (
      <div
        className={`w-full h-64 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg flex items-center justify-center ${className}`}
      >
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2 pt-2">
              <Button onClick={() => onPlay(recording)} className="flex-1">
...
export const DemoRecordings = ({ sessionId, isOpen, onClose }) => {
    ...
    try {
      ...
      if (videoUrl) {
        ...
        if (response.ok) {
          ...
          link.href = url;
          link.download =
            recording.fileName || `recording-${recording.id}.webm`;
  ...
  const handleDeleteRecording = async (recording) => {
    ...
    try {
      const response = await authApiService.deleteDemoRecording(recording.id);

      if (response.success) {
        setRecordings((prev) => prev.filter((r) => r.id !== recording.id));
        toast.success(
          "Recording Deleted",
          "Recording has been deleted successfully"
        );
      } else {
        toast.error(
          "Delete Error",
          response.message || "Failed to delete recording"
        );
      }
    } catch (error) {
  ...
  const handleVideoError = (error) => {
    ...
    const errorMessage =
      "Failed to load video. The file may be corrupted or the server may be unavailable.";
        ...
        >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  View and manage recordings for this demo session (
            ...
            <div className="flex-1 p-6 overflow-y-auto">
              {isLoading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <Button onClick={loadRecordings}>Try Again</Button>
                </div>
              ) : recordings.length === 0 ? (
                <div className="text-center py-12">
                ...
                <div className="mb-4">
                  <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">
                    {selectedRecording.fileName ||
                      `Recording ${selectedRecording.id?.slice(-8)}`}
                  </h3>
                  <div className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
                    ...
                    <p>
                      Size:{" "}
                      {(selectedRecording.fileSize / 1024 / 1024).toFixed(2)} MB
                    </p>
                  </div>
                </div>
                ...
                {getVideoUrl(selectedRecording) ? (
                  <div className="space-y-4">
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400 bg-blue-50 dark:bg-blue-900/20 p-2 rounded">
                      <div className="grid grid-cols-2 gap-2">
                        <span>
                          <strong>File:</strong> {selectedRecording.fileName}
                        </span>
                        <span>
                        ...
                        <span>
                          <strong>Size:</strong>{" "}
                          {(selectedRecording.fileSize / 1024 / 1024).toFixed(
                            2
                          )}{" "}
                          MB
                        </span>
          ...
          <div className="flex items-center justify-between p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50">
            <div className="text-sm text-gray-600 dark:text-gray-400">
              💡 Tip: Click on any recording to preview it. Use download button
              for offline access.
            </div>

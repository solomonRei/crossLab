...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    if (!state.isAuthenticated || !state.token) {
      devLog('Token expiration check: Skipping - not authenticated or no token', {
    ...
    const checkTokenExpiration = () => {
      ...
      devLog('Token expiration check:', {
        ...
        isCurrentTokenExpired: currentToken ? isTokenExpired(currentToken) : 'no token'
      })
  ...
  const login = async (email, password) => {
    ...
    try {
      ...
      if (response.success && response.data && response.data.token && response.data.user) {
        ...
        devLog('AuthContext: Validation failed, missing fields:', missingFields)
